$(document).ready(function() {
    ...
    if(document.getElementsByName("player_search").length > 0) {
        ...
        futsearch.autocomplete({
            ...
            return $("<li></li>").data("item.autocomplete", item).append('<a data-id="' + item.id + '" data-slug="' + item.slug + '"><img class="clubpicture" src="http://cdn.futbin.com/content/fifa19/img/clubs/' + item.club + '.png" /><img class="nationpicture" src="http://cdn.futbin.com/content/fifa19/img/nation/' + item.nation + '.png" /><img class="playerpicture" src="http://cdn.futbin.com/content/fifa19/img/players/' + item.base_id + '.png" /><span class="name">' + item.short_name + '</span> (' + item.position + ') <span class="rating rating-search ut19 ' + rating_color + ' ' + get_revision_display(revision) + '">' + item.rating + '</span></a>').appendTo(ul);
        };
    }
